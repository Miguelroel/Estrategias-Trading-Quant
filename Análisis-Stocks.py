...
def MACD(symbol, interval, series_type, fastperiod = 12, slowperiod = 26, signalperiod = 9):
    function = "MACD"
    url = "https://www.alphavantage.co/query"
    parametros = {"function": function, "symbol": symbol, "interval": interval, "series_type": series_type,
                  "fastperiod": fastperiod, "slowperiod": slowperiod, "signalperiod": signalperiod, "apikey": TOKEN}
...
def RSI(symbol, interval, series_type = "close", time_period = 14):
    function = "RSI"
    url = "https://www.alphavantage.co/query"
...
def tabla(symbol, mean_volume, fast_ema, slow_ema):
    data = getDailyAdj(symbol, "full")
    data = data[["Open", "High", "Low", "AdjClose", "Volume"]]
    data["Volume Mean"] = data.Volume.ewm(span = mean_volume).mean()
    macd = MACD(symbol, "daily", "close")
    rsi = RSI(symbol, "daily")
    fema = EMA(symbol, fast_ema)
    sema = EMA(symbol, slow_ema)
    dataTot = pd.concat([data, macd, rsi, fema, sema], axis = 1)
